...
def replace_conv(model, ghost_module, arc=None, pretrain=False, cheap_pretrian=False, point_conv=False):
    ...
    :return:
    """
    assert (arc in ("resnet56", "vgg16","regnet"))
    count=0
    ...
    for idx, (name, sub_m) in enumerate(model.named_modules()):
        ...
        if in_c>3:
            if count>30:
                new_conv = ghost_module(in_c, out_c, kernel_size=[3,5,7,9],stride=stride)
                print("testtesttest")
            else:
                new_conv = ghost_module(in_c, out_c, kernel_size=[3,5,7,9],stride=stride)
        else:
            new_conv = nn.Conv2d(in_c, out_c, k_s,stride=stride)
            print("***")
        # 先统一用kaiming初始化
